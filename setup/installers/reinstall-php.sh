...
ensure_installed "phpenv_version-${PHP_VERSION_INSTALL}" "PHP version [${PHP_VERSION_INSTALL}] is not already installed. Please use the install script."
...
log_info "Stopping previous version of PHP-FPM."

service php-fpm-${PHP_VERSION_INSTALL} stop

log_info "Removing previous version of PHP installed at ${PHP_VERSION_ROOT}."

mv "${PHP_VERSION_ROOT}" "${PHP_VERSION_ROOT}-$(date +%Y%m%d%H%M%S)"
...
log_info "PHP version ${PHP_VERSION_INSTALL} reinstall complete!"
